...
// Ensure Firebase anonymous auth before writes
...
export const saveResume = async (base64Data) => {
  if (!isFirebaseConfigured || !storage || !db) {
    console.warn("Firebase not configured. Cannot upload CV.");
    return false;
  }
  await ensureAuth();
  try {
...
export const resetPortfolio = async () => {
  ...
  try {
    ...
    console.error("Error resetting portfolio in Firebase:", e);
    return false;
  }
};
